    ...
    public class GraficDAL
    {
        ...
        public static List<GraficTO> GetGraficMes(int intMes, UsersTO clsUser, string strConnection, string strLoja)
        {
            ...
            try
            {
                ...
                strSQL.Append(" GROUP BY base_clientes.Razao_Social,base_clientes.Cnpj, base_clientes.Mes, produtos_base.Grupo, produtos_base.Sub_Consultoria ");
                strSQL.Append(" UNION ");
                ...
                strSQL.Append(" FROM base_clientes ");
                strSQL.Append(" INNER JOIN produtos_base ON base_clientes.Barras = produtos_base.CodBarra ");
                strSQL.Append(" WHERE produtos_base.Grupo IN ('Propagados', 'Alternativos' , 'Genéricos') AND base_clientes.Mes = @Mes GROUP BY base_clientes.Cnpj, base_clientes.Mes) AS xTemp WHERE Mes > 0");
